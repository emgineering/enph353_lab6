...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        near_path = grayscaled[-10::]
        far_path = grayscaled[-60:-50]
        _, mask_near = cv2.threshold(near_path, 130, 255, cv2.THRESH_BINARY_INV)
        _, mask_far = cv2.threshold(far_path, 130, 255, cv2.THRESH_BINARY_INV)

        M_near = cv2.moments(mask_near)
        M_far = cv2.moments(mask_far)
        if M_near["m00"] != 0:
			self.timeout = 0
			cX = int(M_near["m10"] / M_near["m00"])
			state[int(cX * 10 / width)] = 1
		
        if M_far["m00"] != 0:
			self.timeout = 0
			cX = int(M_far["m10"] / M_far["m00"])
			state[int(cX * 10 / width) + 10] = 1
